...
import itertools
import pssms
from math import log2, log, log10, exp, copysign
...
def signed_regulation_score(pair, scores, phosfun, med_phosfun, reg_sites, signs):
    ...
    for pos, res, score in scores:
        ...
        if kin_b_signs is None or pos not in kin_b_signs:
            hs_sign = "NA"
        else:
    ...
    all_signs = ",".join([str(hs_sign) for hs_sign in hs_signs])
    hs_scores = []
    hs_signs = []
    ...
    # signed_func_scores = [sign * score for (sign, score) in
    #                       zip(hs_signs, hs_scores)]
    ...
    best_score_i = signed_func_scores.index(signed_func_score)
    ...
    if best_dcg == worst_dcg or len(hs_scores) < 2:
        return ("NA", site_score, signed_func_score, sites[best_score_i],
                all_scores, all_signs)
    idcg = sign*(dcg-worst_dcg)/(best_dcg-worst_dcg)
    return (idcg, site_score, signed_func_score, sites[best_score_i],
            all_scores, all_signs)
...
def score_network(kinase_file, out_file):
    ...
    signs = read_sign_file("out/reg-site-bart-sign-preds-rescaled.tsv")
    ...
    with open(out_file, "w") as v:
        v.write("\t".join(["node1", "node2", "kinase.type", "sub.kinase.type",
                           ...
                           "signed.func.score", "max.signed.score.res",
                           "all.pssm.scores", "all.func.scores", "all.signs",
                           "all.sites"])+"\n")
        for pair in pairs:
            if pair[0] == pair[1]:
                continue
            ...
            if residues == set(['Y']):
                (dcg, site_score, signed_func_score, max_func_score_site,
                 all_func_scores, all_signs) = signed_regulation_score(
            ...
            v.write("\t".join([pair[0], pair[1], kin_type, sub_type,
                               str(max_pssm_score), str(dcg), str(site_score),
                               str(signed_func_score), max_func_score_site,
                               all_pssm_scores, all_func_scores, all_signs,
                               all_sites])+"\n")
